	...
	{#if exhibition}
		...
		<div class="wrapper absolute bottom-0 left-0 w-full" transition:fly={{ y: 100, duration: 300 }}>
			...
			>
				{#if unknown}
					<Avatar
						className="flex-shrink-0 justify-center items-center bg-light-cards-background-bg rounded-sm"
				...
				{:else}
					...
					></Avatar>
				{/if}
				<div class="flex w-[70%] flex-col overflow-hidden text-left text-light-text-primary">
					<TextMedium className="w-full overflow-hidden text-ellipsis"
						>{unknown ? m.scan_exhibition_error_heading() : toProperCase([exhibition])}</TextMedium
					>
					<TextSmall
						>{unknown ? m.scan_exhibition_error_description() : m.scan_exhibition()}</TextSmall
					>
				</div>
				{#if !unknown}
					<Icon
